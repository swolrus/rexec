...
int recieve_bfile(int socket, char *filePath) {
    ...
    // write in binary
    char fileData[BUF_SIZE] = {0};
    
    uint32_t nbytes = 0;
    uint32_t total = 0;
    while (total < sz) {
        size_t rec = sz - total;
        if (rec > sizeof(fileData)) {rec = sizeof(fileData);}

        if ((nbytes = recv(socket, &fileData, rec, 0)) < 0)
            exit(error("recv() bfile data failed", EXIT_FAILURE));
        if (( fwrite(&fileData, sizeof (char), nbytes, fp) ) < 0)
...
//  SEND THE FILE LENGTH
    ...
    uint32_t total = 0;
    uint32_t nbytes = 0;
    while ( (nbytes = fread(&fileData, sizeof(char), sizeof(fileData), fp)) > 0 )
    {
        if (( send(socket, &fileData, nbytes, 0)) < 0) // send length
            exit(error("send() failed", EXIT_FAILURE));
        total += nbytes;
        printf("sent: %d - %d/%d\n", nbytes, total, sz);
        memset(fileData, 0, sizeof(fileData));
